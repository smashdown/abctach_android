package com.abctech.blogtalking.module.imagepicker;
...
import com.abctech.blogtalking.module.imagepicker.event.HSEventImageFolderSelected;
import com.abctech.blogtalking.module.imagepicker.model.HSImageItem;
import com.abctech.blogtalking.module.imagepicker.util.MediaStoreImageUtil;
...
public class FrgImagePickerImageList extends BTBaseFragment {
            ...
            Glide.with(getActivity())
                    ...
                    .listener(new RequestListener<File, GlideDrawable>() {
                        ...
                        public boolean onException(Exception e, File model, Target<GlideDrawable> target, boolean isFirstResource) {
                            Log.d(BTApp.LOG_TAG, "failed image model=" + model);
                            e.printStackTrace();
                            return false;
                        }
    ...
    private void updateActionBar(String title) {
        if (!TextUtils.isEmpty(title))
            ((BTBaseActivity) getActivity()).getSupportActionBar().setTitle(title);

        if (mMaxCount > 0)
            ((BTBaseActivity) getActivity()).getSupportActionBar().setSubtitle(String.format(getString(R.string.hs_image_selected_with_value), mSelectedImageItems.size(), mMaxCount));
        else
            ((BTBaseActivity) getActivity()).getSupportActionBar().setSubtitle(null);
    }
